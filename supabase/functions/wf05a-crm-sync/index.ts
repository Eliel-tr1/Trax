...
//
// Expected POST body (exactly WF05a's "הכנת חיפוש לקוח" node's output shape):
// { first_name, last_name, phone_e164, email, page_url, submitted_at,
//   utm: { utm_source, utm_medium, utm_campaign, utm_content, utm_keyword, utm_ref, ... } }
//
...
Deno.serve(async (req: Request) => {
  ...
  const { first_name, last_name, phone_e164, email, page_url, utm } = body;
  if (!phone_e164) return jsonResponse({ error: "phone_e164 is required" }, 400);

  const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);
  const u = utm || {};
  const leadSource = SOURCE_MAP[(u.utm_source || "").toLowerCase()] || (u.utm_source ? "אחר" : null);
  ...
  if (existingCustomers && existingCustomers.length) {
    ...
    // someone who's already progressed past "ליד חדש".
