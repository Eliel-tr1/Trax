...
const PATH = { customer: id => `/customers/${id}`, sale: id => `/sales/${id}` }
const LABEL = { customer: 'לקוח', sale: 'מכירה' }

// Resolves meetings/phone_calls' polymorphic related_type + related_id into
// a clickable "משויך ל" link, using loadOptions()'s cached customers/sales
// lists (no per-row query — see lib/api.js). Renders '-' if the related
// record can't be resolved (e.g. it was deleted).
